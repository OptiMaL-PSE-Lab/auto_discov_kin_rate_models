...
ini_NO = [5e0, 1e1, 5e0, 5e0, 0e0, 18940091e-8, 0e0]
ini_N = [0e0, 0e0, 2e0, 0e0, 2e0, 91338518e-8, 164124533e-8]
ini_O = [0e0, 0e0, 0e0, 3e0, 3e0, 92559262e-8, 109541306e-8]

initial_conditions = [[x0...] for x0 in zip(ini_NO, ini_N, ini_O)]

scoeff = [-1e0, 1e0, 5e-1]

function rate(NO, N, O; k1=2e0, k2=5e0)
    num = (k1*NO^2)
...
datasets = [permutedims(readdlm(project_dir*"/data_DNO_$i.csv", '|', Float64, '\n')) for i in 0:num_initial_conditions-1]
...
options = SymbolicRegression.Options(binary_operators=(+, *, /, -),hofFile="hall_of_fame_DNO.csv",maxsize=18,parsimony=1e-5)
